...
//! /v1/library/ratings (GET) — the gateway-owned durable like/dislike,
//! plus its always-on effect on the recommender (dislike hard-excludes,
//! like boosts) regardless of the preference feature flag.
...
async fn put_then_get_roundtrip() {
    ...
    assert_eq!(ratings[0]["track_id"], "t1");
...
async fn put_null_clears_the_rating() {
    ...
    let status = app
        .oneshot(auth_put(
            "/v1/library/rating",
            &json!({"track_id": "t1", "rating": "dislike"}),
        ))
        .await
        .unwrap()
        .status();
...
#[tokio::test]
async fn put_rejects_empty_track_id() {
    let state = build_state(test_config()).await;
    let status = build_router(state)
        .oneshot(auth_put(
            "/v1/library/rating",
            &json!({"track_id": "", "rating": "like"}),
        ))
        .await
        .unwrap()
        .status();
